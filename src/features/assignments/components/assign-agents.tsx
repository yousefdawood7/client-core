...
import { Agent, INITIAL_AGENTS, COMPANIES } from "../constants";
...
export default function AssignAgents() {
  ...
  const [checkedAvailableIds, setCheckedAvailableIds] = useState<Set<string>>(new Set());
  const [checkedAssignedIds, setCheckedAssignedIds] = useState<Set<string>>(new Set());
...
const selectedCompany =
  ...
  const assignedAgents = useMemo(() => {
    return INITIAL_AGENTS.filter(agent => assignedAgentIds.includes(agent.id));
  }, [assignedAgentIds]);
  ...
  const moveToAvailable = () => {
    ...
    setCompanyAssignments(prev => {
      const currentAssigned = prev[selectedCompanyId] || [];
      const updatedAssigned = currentAssigned.filter(id => !checkedAssignedIds.has(id));
      return {
        ...prev,
  ...
  const handleSaveAgentAssignments = () => {
    toast.success(`Agent assignments saved successfully for ${selectedCompany.name}`);
  };
